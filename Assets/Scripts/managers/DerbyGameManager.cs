...
public class DerbyGameManager : MonoBehaviour
{
    ...
    void ReportSurvivalMissionIfHuman(PlayerEntry winner)
    {
        ...
        {
            MissionManager.Instance?.ReportResult(session.chosenGameMode, session.chosenMap, MissionObjectiveType.SurvivalTime, survivalTime, playerSlotIndex); // ← corregido
        }
    }
}